...
Example:
    from dexim_core.nodes import RobotControlNode
...
from dexim_core.nodes.utils import RateLimiter
...
class RobotControlNode(ManagedNode, ABC):
    ...
    def _smootherstep(t: float) -> float:
        ...
        and acceleration at the boundaries (t=0 and t=1). This creates
        a gentle "S-curve" motion profile ideal for robotics applications.
        ...
        Note:
            - At t=0: returns 0, first derivative=0, second derivative=0
            - At t=1: returns 1, first derivative=0, second derivative=0
            - Creates smooth ease-in-out motion (no jerk at boundaries)
            - Velocity profile is a smooth bell curve
        """
        # Clamp to [0, 1] for safety
        t = np.clip(t, 0.0, 1.0)

        # 5th order polynomial: 6t^5 - 15t^4 + 10t^3
        # Optimized computation: t^3 * (6t^2 - 15t + 10)
    ...
    def _move_to_position_safely(
        ...
        for i in range(num_steps):
            ...
            alpha = self._smootherstep(t)  # Smooth S-curve [0, 1]
    ...
    def _send_command(self, joint_cfgs: np.ndarray):
        ...
        try:
            ...
            from dexim_core.robot_interface import JointCommand
