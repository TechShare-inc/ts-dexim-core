...
class BaseRobotVisualizer(ABC):
    ...
    def update_feature_vectors(self, feature_vectors: dict[str, np.ndarray]) -> None:
        ...
        Note: Subclasses can override this method for custom behavior.
        """
        for frame_name, vector in feature_vectors.items():
            if frame_name in self.feature_handles:
                # Get the base position (this needs to be implemented by subclasses)
                # For now, we'll just log a warning
                if self.verbose:
                    self.logger.warning(
                        f"update_feature_vectors not fully implemented in base class for '{frame_name}'"
                    )
            else:
                if self.verbose:
                    self.logger.warning(
                        f"No feature handle found for frame '{frame_name}'"
                    )
